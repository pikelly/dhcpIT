module DHCP
  ...
  class MS < DHCP::Server
    ...
    def loadSubnets
      ...
      response.each do |line|
        ...
        if line =~ /^\s*([\d\.]+)\s*-\s*([\d\.]+)\s*-\s*(Active|Disabled)/
          network = $1
          netmask = $2
          subnet = DHCP::Subnet.new(self, network, netmask)
        end
      end
    end

    def loadSubnetData subnet
      raise "invalid Subnet" unless subnet.is_a? DHCP::Subnet
      ...
      response.each do |line|
        ...
        if line =~ /^\s+([\w\.]+)\s+-\s+([-a-f\d]+)/
          ip = $1
          mac = $2.gsub(/-/,":").match(/^(.*?).$/)[1]
          DHCP::Record.new(subnet, ip, mac) unless ip.nil? or mac.nil?
        end
      end
      super subnet
      subnet.loaded = true
    ...
    def loadRecordOptions record
      raise "invalid Record" unless record.is_a? DHCP::Record
    ...
    def invoke cmd
      ...
      raise DHCP::Error, "invalid Response - curl return error code #{$?} - while executing #{filtered}" unless $? == 0
    ...
    def validate_response cmd, operand, response
      ...
      raise DHCP::Error.new msg if msg
    rescue
      raise DHCP::Error.new "Unknown error while processing #{msg} - #{response}"
    end
